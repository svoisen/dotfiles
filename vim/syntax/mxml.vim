...
"		modifications to Abdul's work by David Woods
...
"
"   derived from Abdul Qabiz's work
"
" REFERENCES:
...
syn region  xmlString contained start=+"+ end=+"+ contains=xmlEntity, cssColor, actionScriptExpression, asEvent display
syn region  xmlString contained start=+'+ end=+'+ contains=xmlEntity, cssColor, actionScriptExpression, asEvent display
...
if exists('g:mxml_syntax_folding')
	...
	\ contained
	\ contains=xmlError,xmlTagName,xmlAttrib,xmlEqual,xmlString,@xmlStartTagHook,mxmlEvent
...
if main_syntax != 'actionscript' || exists("actionscript")
  ...
  syn region  mxmlScriptTag     contained start=+<mx:Script+ end=+>+       contains=htmlTagN,htmlString,htmlArg,htmlValue,htmlTagError,htmlEvent,mxmlEvent
  MXMLHiLink mxmlScriptTag xmlTag

  syn region asEvent		contained matchgroup=asBracket start=+{+ matchgroup=asBracket end=+}+ keepend contains=@mxmlScript
  "syn region mxmlEvent        contained start=+\<\a\+\s*=[\t ]*'{+hs=e+1 end=+}'+he=s-1 keepend contains=mxmlEventBracket
  "syn region mxmlEvent        contained start=+\<\a\+\s*=[\t ]*"{+hs=e+1 end=+}"+he=s-1 keepend contains=mxmlEventBracket
  "syn region mxmlEventBracket contained start=+"{+ms=e+1 end=+}"+ms=s-1 contains=@mxmlScript
  "MXMLHiLink mxmlEventBracket mxmlEvent
  "MXMLHiLink mxmlEventSQ mxmlEvent
  "MXMLHiLink mxmlEventDQ mxmlEvent

  " an actionscript expression is used as an arg value
  syn region actionScriptExpression contained start=+&{+ keepend end=+};+ contains=@mxmlScript,@htmlPreproc
endif

if main_syntax != 'css' || exists("css")
  syn include @css syntax/as_css.vim
  unlet b:current_syntax
  syn region  Styles start=+<mx:Style[^>]*>+ keepend end=+</mx:Style>+me=s-1 contains=@css,mxmlCssStyleComment
  syn region  cssScriptTag     contained start=+<mx:Script+ end=+>+       contains=htmlTagN,htmlString,htmlArg,htmlValue,htmlTagError,htmlEvent,mxmlEvent
  MXMLHiLink cssScriptTag xmlTag
endif
